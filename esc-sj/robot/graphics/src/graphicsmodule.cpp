 ...
 * Copyright (c) 2010 Manuel Francisco Naranjo <manuel@aircable.net>
 *
...
struct ImageObject : public CActive {
  ...
  void ConstructL(TPtrC8& data, TPtrC8& mime)
  { 
    EnsureFbsSessionIsOpen();
    iDecoder=CImageDecoder::DataNewL(FsSession(),data, mime, 
			(CImageDecoder::TOptions) (CImageDecoder::EOptionNoDither | CImageDecoder::EPreferFastDecode));
    
    const TFrameInfo *frmInfo=&iDecoder->FrameInfo();
    iBitmap=new (ELeave) CFbsBitmap();
    TSize size;
    
    if (frmInfo->iFlags && TFrameInfo::EFullyScaleable){
		PySys_WriteStdout("can scale during loading");
		size=TRect(0,0, 640, 480).Size();
    } else {
		size=frmInfo->iFrameCoordsInPixels.Size();
    }
    
    
    User::LeaveIfError(iBitmap->Create(size,EColor64K));
    iDecoder->Convert(&iStatus, *iBitmap);
    iState=EPendingLoad;
    SetActive();    
  }
  ...
  int StartLoadDataL(TPtrC8& aData) {
      EnsureFbsSessionIsOpen();
      DEBUG_ASSERT(iState == ENormalIdle);
      iDecoder=CImageDecoder::DataNewL(FsSession(),aData);
      const TFrameInfo *frmInfo=&iDecoder->FrameInfo();        
      if (frmInfo->iFrameCoordsInPixels != iBitmap->SizeInPixels()) {
        delete iDecoder;
        return 0;
  ...
  void StartScaleL(CFbsBitmap *aTargetBitmap,int aMaintainAspectRatio=EFalse) {
    ...
    iScaler->DisablePostProcessing(true);
    iScaler->SetQualityAlgorithm(CBitmapScaler::EMinimumQuality);
    iScaler->UseLowMemoryAlgorithm(true);
...
/*
static void
pyPySys_WriteStdout(const char *format, ...)
{
  va_list args;
  va_start(args,format);
  char buf[128];
  // Unfortunately Symbian doesn't seem to support the vsnPySys_WriteStdout
  ...
  vsPySys_WriteStdout(buf, format, args);    
  char buf2[128];
  sPySys_WriteStdout(buf2, "print '%s'", buf);
...
extern "C" PyObject *
graphics_ImageFromBuffer(PyObject* /*self*/, PyObject* args)
{
  PyObject *object;
  PyObject *callback=NULL;
  PyObject *mime;
  Py_ssize_t mime_length;
  Image_object *obj;
  if (!PyArg_ParseTuple(args, "Os#|O", &object, &mime, &mime_length, &callback)) 
	return NULL;
  
  unsigned char * data;
  Py_ssize_t data_length;
  if (PyObject_AsReadBuffer(object, (const void **)&data, &data_length)!=0)
	  return PyErr_NoMemory();
  
  TPtrC8 dataPtr( (TUint8*)data, data_length);
  
  if (!(obj = PyObject_New(Image_object, Image_type))) 
    return PyErr_NoMemory();

  // Make a Symbian descriptor pointer to the char * mimeStr
  TPtrC8 mimePtr( (TUint8*)mime, mime_length );
  ...
  TRAPD(error, {
      ...
      if (callback)
			  obj->data->SetCompletionCallback(callback);
      obj->data->ConstructL(dataPtr, mimePtr);
      CleanupStack::Pop();
    });
  if(error != KErrNone){
    PyObject_Del(obj);
...
extern "C" PyObject*
Image_data(Image_object *self, PyObject *args)
{
  PyObject *data;
  Py_ssize_t data_length;
  ImageObject *image=self->data;
  PyObject *callback=NULL;
  int sizeMatches=1;
  if (!PyArg_ParseTuple(args, "s#O", &data, &data_length, &callback)) 
    return NULL;
  ...
  TRAPD(error, {
      if (callback)
	image->SetCompletionCallback(callback);      
      sizeMatches=image->StartLoadDataL(dataPtr);      
    });
  if (!error && !sizeMatches) {
    PyErr_SetString(PyExc_RuntimeError, "file size doesn't match image size");
    return NULL;
...
graphics_screenshot(PyObject* /*self*/)
{
  ...
  const static PyMethodDef Image_methods[] = {
    ...
    {"data", (PyCFunction)Image_data, METH_VARARGS},
  ...
  static const PyMethodDef graphics_methods[] = {
    ...
    {"ImageFromBuffer", (PyCFunction)graphics_ImageFromBuffer, METH_VARARGS, NULL},
...
#define DEFTYPE(name,type_template)  do {				\
  ...
  extern "C" {
  DL_EXPORT(void) init_airbotgraphics(void)
  {
    PyObject *m;
    ...
    //RFbsSession::Connect();
    m = Py_InitModule("_airbotgraphics", (PyMethodDef*)graphics_methods);
