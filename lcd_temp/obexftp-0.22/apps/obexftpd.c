...
static int fileNameNumber=0;

uint32_t connection_id = 0;

int verbose = 0;

#define DEBUG(format, args...) if (verbose) fprintf(stderr, "%s(): " format, __FUNCTION__, ##args);
...
static void get_server(obex_t *handle, obex_object_t *object)
{
	...
	{
		DEBUG("Got a GET without a name-header!\n");
		OBEX_ObjectSetRsp(object, OBEX_RSP_NOT_FOUND, OBEX_RSP_NOT_FOUND);
		return;
	}
	//fprintf(stderr, "%s:%d:%s\n", __FILE__, __LINE__, __FUNCTION__);
...
static int safe_save_file(char *name, const uint8_t *buf, int len)
{
	...
	snprintf(filename, 255, "%s_%X", name, fileNameNumber);

	DEBUG("Filename = %s\n", filename);

	fileNameNumber++;
	
	if (fileNameNumber > 255)
	    fileNameNumber=0;
...
static void put_done(obex_t *handle, obex_object_t *object, int final)
{
	...
	if(!body)	{
		DEBUG("Got a PUT without a body\n");
		OBEX_ObjectSetRsp(object, OBEX_RSP_CONTINUE, OBEX_RSP_SUCCESS);
	}
	if(!name)	{
		name = strdup("OBEX_PUT_Unknown_object");
		DEBUG("Got a PUT without a name. Setting name to %s\n", name);
...
static void server_request(obex_t *handle, obex_object_t *object, int UNUSED(event), int cmd)
{
	...
	case OBEX_CMD_SETPATH:
		DEBUG("Received SETPATH command\n");
		OBEX_ObjectSetRsp(object, OBEX_RSP_CONTINUE, OBEX_RSP_SUCCESS);
...
static void obex_event(obex_t *handle, obex_object_t *obj, int mode, int event, int obex_cmd, int obex_rsp)
{
	...
	case OBEX_EV_STREAMAVAIL:
       	DEBUG("Time to read some data from stream\n");
        break;
	...
	case OBEX_EV_REQCHECK:
		/* e.g. mode=01, obex_cmd=03, obex_rsp=00 */
            	DEBUG("%s() OBEX_EV_REQCHECK: mode=%02x, obex_cmd=%02x, obex_rsp=%02x\n", __func__, 
	...
	case OBEX_EV_REQDONE:
        	...
        	if(obex_rsp == OBEX_RSP_SUCCESS)
	            ...
	            success = FALSE;
        	    DEBUG("OBEX_EV_REQDONE: obex_rsp=%02x\n", obex_rsp);
	        }
		break;
	...
	case OBEX_EV_ABORT:
		/* Request was aborted */
            	DEBUG("OBEX_EV_ABORT: mode=%02x, obex_cmd=%02x, obex_rsp=%02x\n", 
				mode, obex_cmd, obex_rsp);
		break;
