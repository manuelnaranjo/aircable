...
public class Code {
	...
	public static void main(String[] args) throws IOException{
		...
		if (args[0].equals("f")){
			...
			if (args.length!=3){
				System.out.println(
						"Usage:\n" +
						"\tmake In_Folder Out_Folder");
				System.exit(0);
			}
			
			String base = parseFile(readFile(args[1]+ System.getProperty("file.separator")+"AIRcable.bas"));
			...
			for (int i = 0 ; i< dirs.length ; i++){
				if (dirs[i].isDirectory() && !dirs[i].isHidden() && !dirs[i].getName().contains("uploader")){
					String replace = base;
					String diff = parseFile(readFile(dirs[i].toString()+System.getProperty("file.separator")+"AIRcable.bas"));
					
					StringTokenizer diffLines = new StringTokenizer(diff,"\r\n");
					...
					new File(outFolder.toString()+ System.getProperty("file.separator") + 
							dirs[i].getName() + System.getProperty("file.separator"))
							.mkdirs();
	...
	public static String parseFile(String in){
		...
		while (lines.hasMoreElements()){
			String line = parseLine(lines.nextToken().trim());
			if (line==null || line.trim().isEmpty())
				continue;
			
			if (line.length() > 32)				
				System.err.printf("Line %s is %d chars long, lines can't be more than 32 chars %n", line, line.length());
			
			if (line.startsWith("@"))
				interrupts.append(line+"\r\n");
			
			else
				out.append(line+"\r\n");					
		}
		
		return (interrupts.toString() + out.toString()).trim()+"\r\n";
	}
	...
	protected static String parseLine(String in){
		...
		if (in.startsWith("0 REM")) return null;
		
		for (int i = 0 ; i < in.length(); i++)
			if (!ready && in.charAt(i)!=' ')
				out+=in.charAt(i);
			else if (!ready && in.charAt(i)==' '){
				out+=in.charAt(i);
				ready = true;
			}
			else if (in.charAt(i)=='"'){
